...
Page:
<?php
echo '<a href='.ROOT.'questions/>Recent</a>||<a href='.ROOT.'questions/highest/>Highest Bid</a><br>';
if($this->topic==null) {
    $this->topic = 0;
}
foreach($this->pagination as $page) {
	if($this->topic!=null) {
		echo '<a href='.ROOT.'questions/unanswered/'.str_replace(',', '', $page).'/'.$this->topic.'/>'.$page.'</a> ';
	} else {
		echo '<a href='.ROOT.'questions/unanswered/'.str_replace(',', '', $page).'/0/>'.$page.'</a> '; //Topic 0 = all topics, at least we'll just go with that?
	}
}
...
function sortTopics(topic) {
	location.href="<?php echo ROOT.'questions/unanswered/1/';?>" + topic + "<?php echo '/'.$this->bid;?>";
}
</script>